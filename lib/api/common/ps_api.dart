...
abstract class PsApi {
  ...
  Future<PsResource<R>> getServerCall<T extends PsObject<dynamic>, R>(
    ...
    Future<PsResource<R>> getServerCall2<T extends PsObject<dynamic>, R>(
      T obj, String url, hashMap) async {
    try {
        if (!(hashMap is Map)) {
          final List<T> tList = <T>[];
          hashMap.forEach((dynamic data) {
 ...
 Future<PsResource<R>> postData<T extends PsObject<dynamic>, R>(
  ...
  Future<PsResource<R>> postData2<T extends PsObject<dynamic>, R>(
      T obj, String url, hashMap) async {
    try {
        if (!(hashMap is Map)) {
          final List<T> tList = <T>[];
